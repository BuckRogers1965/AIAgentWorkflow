...
# --- Copied classes to make this file self-contained ---
...
- Inputs: These are required parameters.
- Optional Inputs: These are parameters that are not required.
- Outputs: These are the names of the variables that your agent will produce.
"""
...
class WorkflowEditorFrame(BaseEditorFrame):
    ...
    def _on_drop(self, event):
        ...
        if self.drag_data["drop_index"] != -1:
            if self.drag_data["source"] == "agent_list": self.insert_agent_as_step(self.drag_data["payload"], self.drag_data["drop_index"])
    ...
    # --- Selection and Refactoring Logic ---
    ...
    def _perform_refactoring(self, new_name, block_inputs, block_outputs):
        ...
        # ** THE FIX: Directly modify the config dictionary **
    ...
    # --- Tab Creation and Data Handling ---
    ...
    def create_steps_tab(self, tab):
        ...
        self.steps_frame.bind("<Button-1>", self._clear_selection); self.steps_frame._parent_canvas.bind("<Button-1>", self._clear_selection)
        self.refresh_steps_list()
    ...
    def refresh_steps_list(self):
        ...
        for i, step in enumerate(steps):
            ...
            remove_btn = ctk.CTkButton(step_frame, text="X", width=30, fg_color="#D32F2F", hover_color="#B71C1C", command=lambda index=i: self.remove_step(index)); remove_btn.grid(row=0, column=2, padx=5, pady=5)
            ...
            for widget in [step_frame, step_label]:
                ...
                # Use Control/Command for single selection, Shift for range
                widget.bind("<Control-Button-1>", lambda e, index=i: self._on_step_select_click(e, index))
                widget.bind("<Command-Button-1>", lambda e, index=i: self._on_step_select_click(e, index)) # For Mac
                widget.bind("<Shift-Button-1>", lambda e, index=i: self._on_step_select_click(e, index))
                # Right-click for context menu
                widget.bind("<Button-2>", lambda e, index=i: self._on_step_right_click(e, index)) # Linux
                widget.bind("<Button-3>", lambda e, index=i: self._on_step_right_click(e, index)) # Windows/Mac
